...
renderSelect(getSelectExercise, exerciseList_store, 'exercise'); // рендер селекта с упражнениями
...
btnExercise.addEventListener('click', event => {
	...
	let exerciseGet = `<h3 class="cardItem">${
		...
		!inputValue(getInputExercise)
			? getSelectExercise.value
			: getInputExercise.value
	}</h3>   
	...
	renderSelect(getSelectSets, numberOfSets_store, 'set'); //закидываю в селектор подходы
...
`;
		...
		renderSelect(getSelectRepetitions, numberOfRepetitions_store, 'repetition'); // закидываю в селектор повторения
		...
		btnReady.addEventListener('click', () => {
			...
			renderSelect(getSelectExercise, exerciseList_store, 'exercise');
		});
	});
});
